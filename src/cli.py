...
def discount_specific_student():
    ...
    print("-" * 40)
    for idx, (student_id, s) in enumerate(students.items(), start=1):
        print(f"{idx}) Name: {student_id.title()} | Total Cost: Rs.{s.get('total_cost', 0)} | Current Pending: Rs.{s.get('pending_amount', 0.0)}")
    print("-" * 40)
        ...
        if choice == '1':
            ...
            if show_all == 'y':
                ...
                for s_name, info in db.get("students", {}).items():
                    ...
                    if pending > 0:
                        print(f" - {s_name}: Rs.{pending} pending")
                print("-" * 40)
            ...
            if students:
                ...
                for s_name, info in students.items():
                    print(f"Student Name: {s_name} | Remaining Balance: Rs.{info.get('pending_amount', 0.0)}💰")
...
def show_status():
    while True:
        ...
        for idx, (student_id, s) in enumerate(students.items(), start=1):
            ...
            print(f"{idx}) Name: {s.get('name', student_id)} | Status: {s.get('status', 'unknown')}")
...
def add_cash():
    while True:
        ...
        for idx, (student_id, s) in enumerate(students.items(), start=1):
            student_mapping[str(idx)] = student_id
            print(f"{idx}) Name: {s.get('name', student_id)} | Pending Owed: Rs.{s.get('pending_amount', 0.0)}💰")
